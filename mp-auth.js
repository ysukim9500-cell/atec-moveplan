    ...
    return fetch(SB_URL + '/auth/v1/token?grant_type=password', {
      ...
      return r.json().then(function (j) {
        ...
        return loadProfile().then(function (p) {
          ...
          /* 이동계획 소속(mp_members)은 여기서 확인하지 않는다.
             지금 토큰은 aal1 이고 mp_members 의 읽기 정책은 mp_can_read() = aal2 를
             요구한다 — 등록된 사람도 이 시점에는 0행이 나온다.
             그래서 2단계 인증을 통과한 뒤(verify)에 확인한다. */
        ...
        return r.json().then(function (cj) {
          ...
          return authFetch(SB_URL + '/auth/v1/factors/' + fid + '/verify', {
            ...
            return vr.json().then(function (vj) {
              ...
              del(K_FA);
              /* 이제야 mp_members 를 읽을 수 있다. 이동계획 접근 여부는 여기서 정한다 —
                 공용 profiles.perms 를 건드리면 다른 시스템에 영향이 가기 때문이다.
